...
void MMA8451Q::begin(bool highres, uint8_t scale)
{
  _highres = highres;
  ...
  if (_scale == 4 || _scale == 8)
    Wire.write((_scale == 4) ? RANGE_4G : RANGE_8G);
  else // Default to 2g mode
    Wire.write((uint8_t) RANGE_2G);
  Wire.endTransmission();
    ...
    datain = _read_register(XYZ_DATA_CFG);
    Serial.print("XYZ_DATA_CFG: ");
    Serial.println(datain, HEX);    
...
/*
    Serial.print("CTRL_REG1: ");
    Serial.println(get_CTRL_REG1(), HEX);    
*/
/*
    datain = _read_register(CTRL_REG1);
    Serial.print("2nd CTRL_REG1: ");
    Serial.println(datain, HEX);
*/
...
void MMA8451Q::update()
{
  ...
  if (Wire.available()) 
  {
    ...
    if(_highres)
    {
      ...
      _xf = ((float) _xi / 64) * _step_factor;
      _yf = ((float) _yi / 64) * _step_factor;
      _zf = ((float) _zi / 64) * _step_factor;
      */
      _xi = (int16_t)((Wire.read() << 8) + Wire.read());
      _yi = (int16_t)((Wire.read() << 8) + Wire.read());
      _zi = (int16_t)((Wire.read() << 8) + Wire.read());
 ...
 */
bool MMA8451Q::setDebounceCounter(uint8_t n)
{
  _standby();
